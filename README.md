...
- `mutsuki-bot-web-console`: 嵌入式 Bot 管理台装配（WebHost + control/overview/config/upgrade extensions）；另提供 [`build_standalone_console_host`](crates/mutsuki-bot-web-console/src/standalone.rs)（Standalone + `local://` / `quic://` Link；`quic://` 需注入 TLS identity，缺失时结构化失败）
